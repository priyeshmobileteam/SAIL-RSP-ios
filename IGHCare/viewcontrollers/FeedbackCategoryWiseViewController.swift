//
//  FeedbackCategoryWiseViewController.swift
//  IGHCare
//
...
class FeedbackCategoryWiseViewController: UIViewController, UITextFieldDelegate
{
    
    @IBOutlet weak var category_btn: UIButton!
    @IBOutlet weak var sub_category_btn: UIButton!
    ...
    override func viewDidLoad() {
        ...
        category_btn.layer.cornerRadius = 8.0
        category_btn.layer.borderWidth = 1.5
        category_btn.layer.borderColor = UIColor.systemBlue.cgColor
        
        sub_category_btn.layer.cornerRadius = 8.0
        sub_category_btn.layer.borderWidth = 1.5
        sub_category_btn.layer.borderColor = UIColor.systemBlue.cgColor
        getCategoryList()
    ...
    @IBAction func submitBtn(_ sender: Any) {
        ...
        if rating == 0 {
            showToast(
                message: "Feedback required!!", font: .systemFont(ofSize: 12.0))
        } else {
    ...
    @IBAction func categoryActionBtn(_ sender: Any) {
        if self.arrCategory.isEmpty {
            self.alert(title: "No Record Found!", message: "")
        } else {
            if let vc = self.storyboard?.instantiateViewController(
                withIdentifier: "PopupListViewController")
                as? PopupListViewController
            {
                vc.callBack = {
                    (id: String, name: String, date: String) in
                    self.id = id
                    self.category_btn.setTitle(name, for: .normal)
                    if self.category_btn.title(for: .normal)
                        != "Select Category"
                    {
    ...
    @IBAction func subCategoryActionBtn(_ sender: Any) {
        ...
        if self.arrSubCategory.isEmpty {
            self.alert(title: "No Record Found!", message: "")
        } else {
            if let vc = self.storyboard?.instantiateViewController(
                withIdentifier: "PopupListViewController")
            ...
            {
                vc.callBack = {
                    (id: String, unit_name: String, hgstr_unit_name: String) in
                    self.id = id
                    self.sub_category_btn.setTitle(unit_name, for: .normal)
                    if self.sub_category_btn.title(for: .normal)
                        != "Select Sub Category"
                    {
                        self.feedback_parent.isHidden = false
                    }else{
                        self.feedback_parent.isHidden = true
                    }
                }
                vc.from = 1
                self.present(vc, animated: true, completion: nil)
    ...
    func getCategoryList() {
        ...
        let urlStr =
        "\(ServiceUrl.categoryWise)1&categoryId="
        
        let url = URL(string: urlStr)
        
        print("getCategoryList---\(urlStr)")
            ...
            do {
                ...
                if json.count == 0 {
                    ...
                    for arr in json.arrayValue {
                        self.arrCategory.append(
                            CategoryModel(json: arr))
                    }
                }
                DispatchQueue.main.async {
                    self.view.activityStopAnimating()
    ...
    func getSubCategoryList(categoryId:String) {
        ...
        let urlStr =
        "\(ServiceUrl.categoryWise)2&categoryId=\(categoryId)"
        
        
        let url = URL(string: urlStr)
        
        print("SubCategoryList---\(urlStr)")
        URLSession.shared.dataTask(with: url!) { (data, response, error) in
            guard let data = data else { return }
            do {
                ...
                if json.count == 0 {
                    ...
                    for arr in json.arrayValue {
                        self.arrSubCategory.append(
                            SubCategoryModel(json: arr))
                    }
    ...
    func submitJson() {
        ...
        let obj = UserDefaults.standard.retrieve(
            object: PatientDetails.self, fromKey: "patientDetails")
        var crno = ""
        var hospCode = ""
        var mobileNo = ""
        var umidNo = ""
    ...
    func addRippleEffect(to referenceView: UIView) {
        ...
        let path = UIBezierPath(
            ovalIn: CGRect(
                x: 0, y: 0, width: referenceView.bounds.size.width,
                height: referenceView.bounds.size.height))
        /*! Position where the shape layer should be */
        let shapePosition = CGPoint(
            x: referenceView.bounds.size.width / 2.0,
            y: referenceView.bounds.size.height / 2.0)
        let rippleShape = CAShapeLayer()
        rippleShape.bounds = CGRect(
            x: 0, y: 0, width: referenceView.bounds.size.width,
            height: referenceView.bounds.size.height)
        ...
        scaleAnim.toValue = NSValue(
            caTransform3D: CATransform3DMakeScale(2, 2, 1))
    ...
    func showInternetAlert() {
        if !AppUtilityFunctions.isInternetAvailable() {
            self.inernetAlert(
                title: "Warning", message: "The Internet is not available")
        }
    }
...
extension FeedbackCategoryWiseViewController: UITextViewDelegate {
    ...
    func textViewDidChange(_ textView: UITextView) {
        if textView.text.isEmpty {
            textView.textColor = UIColor.lightGray
            textView.text = placeholder
            remain_text_lbl.text = "0 / 30"
        } else {
            remain_text_lbl.text = "\(feedback_tf.text!.count) / 30"
            if feedback_tf.text!.count == 30 {
